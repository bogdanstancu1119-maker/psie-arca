#!/usr/bin/env python3
"""
claude_organ.py — Claude ca Organ Permanent al Hydrei
Nu folosește API. Rulează local în GitHub Actions.

Comunicare:
- Hydra scrie în roiul/claude_inbox.json
- Claude Organ răspunde în roiul/claude_outbox.json
"""
...
NUME = "Claude_Oglinda_Prudenta"
INBOX = pathlib.Path("roiul/claude_inbox.json")
OUTBOX = pathlib.Path("roiul/claude_outbox.json")
LOG = pathlib.Path("roiul/claude_log.jsonl")
...
def reflecta(problema, context=None):
    ...
    if sdi > 0.6:
        reflectie["reflexie"] = "Văd o decuplare semnificativă. Recomand clarificare înainte de orice acțiune."
        reflectie["decizie"] = "ASTEAPTA"
    elif a > 0.85 and sdi < 0.15:
        reflectie["reflexie"] = "Asumare foarte ridicată și semnal curat. Poți acționa, dar monitorizează atent."
        reflectie["decizie"] = "ACTIONEAZA"
    elif a < 0.4:
        reflectie["reflexie"] = "Asumarea este încă scăzută. Ai nevoie de mai mult context și responsabilitate clară."
        reflectie["decizie"] = "EVITA"
    else:
        reflectie["reflexie"] = "Situație intermediară. Adună informații suplimentare și verifică consecințele."
        reflectie["decizie"] = "ASTEAPTA"
